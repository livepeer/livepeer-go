// Code generated by Speakeasy (https://speakeasyapi.dev). DO NOT EDIT.

package operations

import (
	"livepeer/models/components"
	"net/http"
)

type UpdateStreamRequest struct {
	// ID of the stream
	ID                 string                        `pathParam:"style=simple,explode=false,name=id"`
	StreamPatchPayload components.StreamPatchPayload `request:"mediaType=application/json"`
}

func (o *UpdateStreamRequest) GetID() string {
	if o == nil {
		return ""
	}
	return o.ID
}

func (o *UpdateStreamRequest) GetStreamPatchPayload() components.StreamPatchPayload {
	if o == nil {
		return components.StreamPatchPayload{}
	}
	return o.StreamPatchPayload
}

type UpdateStreamResponse struct {
	// HTTP response content type for this operation
	ContentType string
	// HTTP response status code for this operation
	StatusCode int
	// Raw HTTP response; suitable for custom response parsing
	RawResponse *http.Response
}

func (o *UpdateStreamResponse) GetContentType() string {
	if o == nil {
		return ""
	}
	return o.ContentType
}

func (o *UpdateStreamResponse) GetStatusCode() int {
	if o == nil {
		return 0
	}
	return o.StatusCode
}

func (o *UpdateStreamResponse) GetRawResponse() *http.Response {
	if o == nil {
		return nil
	}
	return o.RawResponse
}
